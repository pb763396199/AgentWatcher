...
1. 完成 v0.1.5 新版本发布：ZCode provider（扫描/预览/接续导出，跳转暂不实现）、接续上下文归属守卫与最终 artifact 核对。（已完成，v0.1.5 已发布）
2. 测试基建：真实会话超量时（如 119 张卡触发滚动队列轮换，目标卡被 `hidden` 挤出可视区）zcode-session / zcode-handoff-context flow 点击不稳定；改进方向：flow 内先切 provider 筛选保证目标卡可见，或用固定 fixture 数据代替真实会话，并加「定位-取数-点击」原子重试。2026-09-14 曾尝试修复未收敛，改动已撤销，需单独立任务。
3. 验证长时间运行时的扫描和 UI 刷新稳定性。
4. 补充窗口位置记忆、托盘入口和全局快捷键。
5. 扩展 VS Code Stable/Insiders 与 100%/125%/150% DPI 验证矩阵。
