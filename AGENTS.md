...
- `scan_sessions({ options?: ScanOptions })` → `AgentSession[]`。`ScanOptions` 字段：`maxSessions`、`activeWindowDays`（夹在 1 到 30 之间）、`hideArchived`、`includeCopilot`、`includeCopilotCli`、`includeClaude`、`includeCodex`、`includeOpenCode`、`includeZcode`、`workspacePathBlacklist`。默认最大 80 个（按 provider 各自配额）、活跃窗口 7 天、include 全部默认 `true`。每个会话带 `usage` 字段（可选，`null` 时整体省略）：累计毛输入/缓存读取/输出 token、当前上下文（含窗口上限）、用户轮次、工具调用数、模型调用数、模型名、总跨度时长与活跃时长，OpenCode 另有美元成本、Copilot Chat 另有 credits；数据源没有的项为 `null`。活跃时长口径：Codex/Claude/Copilot Chat/OpenCode 按内容时间戳聚类、相邻间隔超过 `USAGE_ACTIVE_GAP_THRESHOLD_MS`（5 分钟）的空闲不计入（`active_duration_ms_from_timestamps`，OpenCode 走 message 表 SQL）；ZCode 取 `model_usage.duration_ms` 求和（模型真实生成时长）；Copilot CLI 数据源无用量。另有一个懒加载命令 `get_session_usage_detail({ id })` 返回 `{ usage, topTools }`（OpenCode/ZCode 走 SQL，其余源取聚合缓存）。
...
- Bridge 版本：`0.1.12`（与 App 版本独立，只有 Bridge 合同或实现变化时才升级）。
...
- Codex resume/fork 会在 `~/.codex/sessions` 另开一个文件名带 `_<新uuid>` 后缀的 rollout 片段（`session_meta` 仍写原 thread id，ordinal 接续原文件），app-server 的 `thread/list` 会把同一 id 的每个片段各返回一条。`scan_codex_sessions` 对 thread/list 结果按 id 去重、保留 updatedAt 最新的一条；usage 沿片段文件链聚合：事件计数（轮次/工具/模型调用/时长）跨片段求和，token 等累计口径取最新片段。
...
7. 只认最终 artifact，不只认源码。
	...
	- VSIX 要重复安装两次：`code --install-extension artifacts\AgentWatcher\vscode-agentwatcher-bridge\agentwatcher-bridge-0.1.12.vsix --force`。
	- 装完 `code --list-extensions --show-versions | findstr agentwatcher` 只能看到 `agentwatcher.agentwatcher-vscode-session-bridge@0.1.12`，不能有任何 `safe*`。
